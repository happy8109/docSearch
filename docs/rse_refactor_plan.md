...
### 2.1 部署模式 (本地/远程双态)
系统通过 `isRemoteMode` 配置项切换运行逻辑。在“本地模式”下作为数据源并开启 RSE 端口；在“远程模式”下作为查询镜像，透传 SQL。

### 2.2 启动命令支持 (多实例对测)
为了方便在同一台机器上模拟 A/B 网环境进行闭环验证，系统支持通过命令行覆盖配置：

```bash
# 启动实例 A (本地/源端模式，默认端口 3004)
node src/server.js --port=3004

# 启动实例 B (远程/查询模式，自定义端口 3005)
node src/server.js --port=3005 --remote
```

### 2.3 逻辑优先级
1. **命令行参数** (`--port`, `--remote`)
2. **配置文件** `config/app.config.json`
3. **系统内置默认值** (3004, false)
